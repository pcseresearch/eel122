...
//cout<<neg<<" events are negative"<<endl;
	...
	TH2D *top = new TH2D("top", "top", 200,-40,50,200,-50,50);
	TH2D *bot = new TH2D("bottom", "bottom", 400,-40,50,400,-50,50);
...
//main event loop
	...
	if(true){
	TCanvas *c1 = new TCanvas("c1","Plots",40,735,800,600);
	c1->Divide(2,3);
	...
	TCanvas *c2 = new TCanvas("c2","Plots",1370,5,1200,1200);
	c2->Divide(1,2);
//	c2->Divide(1,3);
	c2->cd(1);
	top->Draw("colz");
...
//	bot2->Draw("colz");
	...
	frac = ((double)hit1)/((double)n);
	dfrac = sqrt((double)hit1)/(double)n;
	cout<<endl<<endl<<"Had "<<hit1<<" hits out of "<<n<<" events"<<endl;
	cout<<"Accepted cosmic ray fraction:  "<<frac<<" +/- "<<dfrac<<endl;
	cout<<"Fraction of events lost due to scintillator position: "<<(n-hit1)/hit1<<endl;
