...
#include <TBox.h>
#include <TPolyLine.h>
void calo_plots_cag(Int_t nrun) {
  cout << "Nrun= "<< nrun << endl;
  gROOT->Reset();
  // gStyle->SetOptStat(0);
  //
...
//*************
  ...
  for ( int i = 0; i < Ncalo ; i++) {
    ...
    hadcpedsub[i] = new TH1F(Form("hadcpedsub%02d", i),Form("%s    pedsub adc[%2d];ADC; Counts ",Modname[i],i),100,-100,3000);//range was/is till 1500. 3000 is because M-01-C has a huge gain
    hadccent[i] = new TH1F(Form("hadccent%02d", i),Form("%s    fitted adc;ADC; Counts ",Modname[i]),80,-100,3000);
    hadccut2[i] = new TH1F(Form("hadccut2%02d", i),Form("%s    fitted adc;ADC; Counts ",Modname[i]),100,-100,3000);
...
//run2hit=false;
	...
	for ( int i = 0; i < 5 ; i++) 
	  {
		...
		while(go==true)
		  {//this while loops performs newton's method type fitting loop
    ...
    //place Outer module histograms (in order of original adc)
     ...
     TCanvas *central = new TCanvas("central","Central Module",wtopx, wtopy, ww, wh);
     ...
     gStyle->SetOptFit(1);     
    *    central->cd();
     TF1 *gaus1 = new TF1("gaus1", "gaus",600,1300);

     //for outer modules
          
     Int_t cen_in = 3;//the index of the module
     hadccent[cen_in]->SetStats(1);   
     hadccent[cen_in]->Fit("gaus1","R"); // "R" = fit between "xmin" and "xmax" of the "f1"
     hadccent[cen_in]->SetLineColor(3);
     hadccent[cen_in]->SetMaximum(30000);
     
     hadcpedsub[cen_in]->SetStats(0);   
     hadcpedsub[cen_in]->Draw("same");     
    /          
     //
     /*
     //place middle module histograms
     //     int loc_ind = 5;
     Int_t cen_in = 13;//5=M-02, 9=M-03, 13=M-01, 17=M-04
     //     central->Divide(2,2);
     //    for(int i=0;i<4;i++)
       {
	 TPad * pad1c = new TPad("pad1c","pad1c",0,ydim+sep, xdim-sep, 1);//top-left
	 TPad * pad2c = new TPad("pad2c","pad2c",xdim+sep,ydim+sep,1,1);//top-right
	 TPad * pad3c = new TPad("pad3c","pad3c",0,0, xdim-sep, ydim-sep); //bottom-left
	 TPad * pad4c = new TPad("pad4c","pad4c",xdim+sep,0, 1, ydim-sep);//bottom-right
	 Int_t maxY = 80000;
	 central->cd();
	 ...
	 pad1c->cd();
	 gPad->SetLogy();		
	 TF1 *gaus1 = new TF1("gaus1", "gaus",600,900);

	 hadccent[cen_in]->SetMaximum(maxY);
	 hadccent[cen_in]->Fit("gaus1","R"); // "R" = fit between "xmin" and "xmax" of the "f1"
	 hadccent[cen_in]->SetLineColor(3);
	 hadcpedsub[cen_in]->SetStats(0);   
	 hadcpedsub[cen_in]->Draw("same");	 
	 ...
	 pad2c->Draw();pad2c->cd();gPad->SetLogy();
	 TF1 *gaus1 = new TF1("gaus1", "gaus",650,1050);

	 hadccent[cen_in+1]->SetMaximum(maxY);
	 hadccent[cen_in+1]->Fit("gaus1","R"); // "R" = fit between "xmin" and "xmax" of the "f1"
	 hadccent[cen_in+1]->SetLineColor(3);
	 hadcpedsub[cen_in+1]->SetStats(0);   
	 hadcpedsub[cen_in+1]->Draw("same");	 

	 central->cd();
	 
	 pad3c->Draw();pad3c->cd();gPad->SetLogy();
	 TF1 *gaus1 = new TF1("gaus1", "gaus",750,1100);

	 hadccent[cen_in+2]->SetMaximum(maxY);
	 hadccent[cen_in+2]->Fit("gaus1","R"); // "R" = fit between "xmin" and "xmax" of the "f1"
	 hadccent[cen_in+2]->SetLineColor(3);
	 hadcpedsub[cen_in+2]->SetStats(0);   
	 hadcpedsub[cen_in+2]->Draw("same");	 

	 central->cd();
	 
	 pad4c->Draw();pad4c->cd();gPad->SetLogy();
	 TF1 *gaus1 = new TF1("gaus1", "gaus",200,600);

	 hadccent[cen_in+3]->SetMaximum(maxY);
	 hadccent[cen_in+3]->Fit("gaus1","R"); // "R" = fit between "xmin" and "xmax" of the "f1"
	 hadccent[cen_in+3]->SetLineColor(3);
	 hadcpedsub[cen_in+3]->SetStats(0);   
	 hadcpedsub[cen_in+3]->Draw("same");

       }
     */
     central->Update();     
     central->Print("M-01b.eps");
