...
void calo_plots_slac_xpos(Int_t nrun) {
	cout << "Nrun= "<< nrun << endl;
	gStyle->SetOptStat(1);
...
//      holdypos=(eupper-elower)/abs(eupper-elower)*pow(abs(elower-eupper)/(eupper+emid+elower),0.5)*yposcorr+yposoffset;
      if(eleft >= eright){
		eleftright->Fill( -1.0*(pow((+1.0*(eleft-eright)/(eleft+emid+eright)+es),ep)-pow(es,ep))  );
      ...
      if(elower >= eupper){
		eupdown->Fill(    -1.0*(pow((+1.0*(elower-eupper)/(elower+emid+eupper)+es),ep)-pow(es,ep))  );
...
//				cout<<"||"<< Modname[i]<<"||"<<mean<<"||"<<mean_error<<"||"<<sigma<<"||"<<sigma_error<<"||"<<nphoto<<"||"<<photoerror<< endl;  
 ...
 //*********************************************************
	...
	if(plot_energy_sum){
		...
		hadcesum->Draw();
		TCanvas *xposit = new TCanvas("xposit","X Position",wtopx+10, wtopy-10, ww, wh);
   		xposit->Divide(2,2);
		xposit->cd(1);
		xposition->Draw();
		xposit->cd(2);
