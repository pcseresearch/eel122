...
void decodeEel(int run)
{
  ...
  if (coda != NULL) {                               
    TFile * out = new TFile (Form("data/calotest%d.root",run),"RECREATE"); // creation of the output file                                         
	 ...
	 for ( int i = 0 ; i < size ; i++)
	   {
	     ...
	     val=((buffer[i+index+offset])&0x1fff);
	     //if(evcount<20){cout<< i << " " << offset << " " << evcount<<" "<<chan<<" "<<val<<endl;}
	     adc[chan]=val;
	   }

	 while (index<nevt && ((buffer[index]&0xffffffff)!=0x0d00dd00d))
          {
